...
<style>
  ...
  @media (max-width: 860px) {
    .auth-intro { display: none; } /* hide marketing intro on mobile register */
...
(function () {
  try {
    const raw = localStorage.getItem('geneomx_consumer_portal_v1_split');
    if (!raw) return;
    const saved = JSON.parse(raw);
    const emailEl = document.getElementById('email');
    if (emailEl && !emailEl.value && saved.account?.email && !String(saved.account.email).includes('guest@')) {
      emailEl.value = saved.account.email;
    }
  } catch (e) {}
})();
</script>
