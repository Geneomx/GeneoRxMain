...
return new class extends Migration
{
    ...
    {
        Schema::table('user_profiles', function (Blueprint $table) {
            if (Schema::hasColumn('user_profiles', 'gender')) $table->dropColumn('gender');
            if (Schema::hasColumn('user_profiles', 'pregnant')) $table->dropColumn('pregnant');
            if (Schema::hasColumn('user_profiles', 'kidney_disease')) $table->dropColumn('kidney_disease');
            if (Schema::hasColumn('user_profiles', 'anticoagulants')) $table->dropColumn('anticoagulants');
        });

        Schema::table('check_ins', function (Blueprint $table) {
            if (Schema::hasColumn('check_ins', 'date_checked')) $table->dropColumn('date_checked');
            if (Schema::hasColumn('check_ins', 'adherence_percentage')) $table->dropColumn('adherence_percentage');
        });

        Schema::table('medications', function (Blueprint $table) {
            if (Schema::hasColumn('medications', 'user_id')) $table->dropConstrainedForeignId('user_id');
            if (Schema::hasColumn('medications', 'medication_name')) $table->dropColumn('medication_name');
            if (Schema::hasColumn('medications', 'dosage')) $table->dropColumn('dosage');
            if (Schema::hasColumn('medications', 'duration_months')) $table->dropColumn('duration_months');
        });

        Schema::table('symptoms', function (Blueprint $table) {
            if (Schema::hasColumn('symptoms', 'user_id')) $table->dropConstrainedForeignId('user_id');
            if (Schema::hasColumn('symptoms', 'symptom_name')) $table->dropColumn('symptom_name');
        });
    }
};
